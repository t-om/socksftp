...
FTP.prototype.connect = function (options) {
  ...
  if (this.options.socksproxy === undefined) {
    ...
    //Socks client
    var options = getConnectionOptions(this.options.socksproxy, {
      host: options.host,
      port: options.port
    });
    ...
    Socks.createConnection(options, function (err, soc, info) {
      if (err)
        self.emit('error', makeError(999, 'SOCKS connect timed out'))
      else {
        socket = soc;
        socket.setTimeout(0);
        socket.setKeepAlive(true);
...
FTP.prototype._pasvConnect = function (ip, port, cb) {
  ...
  if (this.options.socksproxy === undefined) {
    ...
    var options = getConnectionOptions(this.options, {
      host: ip,
      port: port
    });
...
function getConnectionOptions(socksproxy, target) {
  var urlObj = new URL(socksproxy);
  var hostname = urlObj.hostname;
  var user = urlObj.username;
  var pass = urlObj.password;
  var proxyPort = urlObj.port || 1080;
  ...
  return {
    ...
    target: {
      host: target.host, // When using bind, it's best to give an estimation of the ip that will be connecting to the newly opened tcp port on the proxy server.
      port: target.port
    }
  };
};